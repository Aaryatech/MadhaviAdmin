...
public class GvnController {
	...
	public ModelAndView getGateGvnDetail(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			hashMap.put(headerId, (ArrayList) gvnGateDetailList);
	...
	public ModelAndView getStoreGvnDetail(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			hashMap.put(headerId, (ArrayList) gvnStoreDetailList);
	...
	public ModelAndView getAccGvnDetail(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			hashMap.put(headerId, (ArrayList) gvnAccDetailList);
		...
		try {
			...
			for (int i = 0; i < gvnAccDetailList.size(); i++) {
				...
				if (gvnAccDetailList.get(i).getGrnGvnStatus() == 6) {
					...
					apporvedAmt=apporvedAmt + gvnAccDetailList.get(i).getAprGrandTotal();

					aprTaxableAmt=aprTaxableAmt+ gvnAccDetailList.get(i).getAprTaxableAmt();
					aprTotalTax=aprTotalTax+ gvnAccDetailList.get(i).getAprTotalTax();
					aprGrandTotal=aprGrandTotal + gvnAccDetailList.get(i).getAprGrandTotal();
					aprCgstRs=aprCgstRs + gvnAccDetailList.get(i).getAprCgstRs();
					aprSgstRs=aprSgstRs + gvnAccDetailList.get(i).getAprSgstRs();
					aprIgstRs=aprIgstRs + gvnAccDetailList.get(i).getAprIgstRs();
					aprROff=aprROff + gvnAccDetailList.get(i).getAprROff();
			...
			try {
				...
				Firebase.sendPushNotifForCommunication(frToken, "",
						"GVN no" + accHeader.getGrngvnSrno() + "punched by you has been cleared by Accounts"
		...
		try {
			...
			try {
				...
				Firebase.sendPushNotifForCommunication(
						frToken, "", "GVN no" + accHeader.getGrngvnSrno()
